        ...
        while (cont_loop==True and failed_starts < restart_limit ):
                ...
                try:
                        try: 
                                ...
                                time.sleep(buffer_wait)
                                (browser.find_element_by_xpath("html/body/div/div[2]/div/form/div[3]/input")).send_keys(USER_NAME)
                                (browser.find_element_by_xpath("html/body/div/div[2]/div/form/div[5]/input")).send_keys(USER_PASSWORD + Keys.RETURN)
                                time.sleep(buffer_wait)
                        ...
                        # Iterate through tasks and pass each to scrape_task() if already in catalog_tasks lists, else just scrape neccessary data 
                                ...
                                try: name = soup.find(name="input", attrs={"id":"sys_readonly.sc_task.number", "class":"form-control disabled "}).attrs['value']
                                except Exception as e: 
                                        name = soup.find(name="input", attrs={"id":"sc_task.number", "name":"sc_task.number"}).attrs['value'] 
                                        print("BS4 Error - Soup cannot access hidden input //input[@id='sys_readonly.sc_task.number'] for task number value, using non-hidden input //input[@id='sc_task.number']")
                                update_index=-1
...
# Scrapes what it can from table, based on whats already set in filter, to establish list of catalog tasks. Sets task limit to number of active tasks if no limit is specified.
        ...
        # Find table body and grab text from each field(td) in each row of table(tr) and assign to data in task obj based on header fields
                ...
                # Grab relevant td tags for each row and set Task attributes, then append Task object to tasks list to return
                        ...
                        task_data= task_data[2:(len(task_data)-2)] # frst of 3 datex is [20:]
...
# Grabs Activity cards from Task page and creates a lists of TaskActivity objects for Tasks, then returns task object with activity list based on update task
        ...
        try: task_number = soup.find(name='input', attrs={'id':'sc_task.number', 'name':'sc_task.number', 'class':'form-control'}).attrs['value']
        except Exception as e: 
                task_number = soup.find(name="input", attrs={"id":"sys_readonly.sc_task.number", "class":"form-control disabled "}).attrs['value']
                print("cannot find //input[@id='sc_task.number'], using hidden input //input[@id='sys_readonly.sc_task.number']")
        ...
        # Grabbing Activity Cards
                ...
                for card in activities:
                        ...
                        date_changed = card.find(name="span", attrs={"class":"sn-card-component-time"}).findChild(name='div', attrs={"class":"date-calendar"}).get_text()
                        change_type = card.find(name="span", attrs={"class":"sn-card-component-time"}).findChildren()[0].get_text()
                        status_changes = card.findChild(name='ul',
                                attrs={"class":"sn-widget-list sn-widget-list-table"}).findChildren("li")
                        task_changes= []
                        # Task Activity Card will either be an Email Sent, a Field changes, or Work notes change type
                        if change_type == "Field changes":
                        ...
                        elif change_type == "Email sent":
                                for li in status_changes[1:len(status_changes)-2]:
                                        spans = li.findChildren()
                                        task_changes += [str(spans[0].get_text()) + " " + str(spans[1].get_text())]
                        activity_cards += [TaskActivity(c, created_by, date_changed, task_changes, change_type)]
                        c += 1
                ...
                # if Task to update is provided just append cards, otherwise just grab necessary information                     
                        ...
                        new_task_attrs = {"number": task_number, 
                                ...
                                "assignment_group": soup.find("input", attrs={"name": "sc_task.assignment_group_label", "id": "sc_task.assignment_group_label" }).attrs['value'],
                                # Note that Requested_for is inside a div with class="hidden"  
                                "requested_for": soup.find("input", attrs={"name": "sc_task.request.requested_for_label", "id": "sc_task.request.requested_for_label" }).attrs['value'],
                                "request_item": soup.find("input", attrs={"name": "sc_task.request_item_label", "id": "sc_task.request_item_label"}).attrs['value'],
                                "request": soup.find("input", attrs={"name": "sc_task.request_label", "id": "sc_task.request_label"}).attrs['value'],
                                # Note that short_description, like task_number, has two versions with this one being the interactable one(class="form-control disabled") and other is right beneath
                                "short_description": soup.find("input", attrs={"class": "form-control disabled", "id": "sys_readonly.sc_task.short_description"}).attrs['value']
                                }
...
# Contains activity information from each card on individual Task Page. Note that a lower index indicates a more recent activity with 0 being the most recent
        def __init__(self, index, created_by, date_changed, changes_list, changes_type):
                self.index = index
        ...
        def get_card(self):
                ...
                for i in self.changes_list: 
                        changes += i + ". "
                return (self.changes_type + " by " + str(self.created_by) + " on " + str(self.date_changed) + " - Effective Changes("+ str(len(self.changes_list)) +"): " + changes)

        def show(self):
                print(f"Activity {self.index} - {self.changes_type}:\n\tUser: {self.created_by}\n \tDate: {self.date_changed}\n \tChanges: {self.changes_list}\n")
